"""Shared fixtures for the frozen feat_graph acceptance suite (Week 2).

FROZEN GOAL HARNESS — do not edit to make a test pass. Per-test environment:
a temp-file SQLite DB with the full agent schema pre-created, keyless provider
settings (so every factory must select its Stub), cleared settings/engine
caches, and the respx fake OpenEMR intercepting outbound FHIR/Standard-API
traffic (host ``openemr.test``).

Feature imports are deliberately NOT done here — each test imports the
``copilot.graph`` surface defensively (see ``_graph_helpers.feature_module``)
so a missing feature is a ran-and-FAILED test, never a collection or setup
error.
"""
...
def _env(tmp_path, monkeypatch):
    ...
    db_file = tmp_path / "acceptance_graph.db"
    ...
    # Keyless everywhere -> the full stub graph must run with no key.
...
@pytest.fixture(autouse=True)
def fake_openemr():
    """Intercept the agent's outbound OpenEMR calls; reset recorded state per test."""
    reset_state()
    with build_router():
        yield
