...
class IntakeWritebackBridge:
    ...
    async def _load_writable_facts(
    ...
    ) -> list[tuple[WriteKind, str]]:
        """Read the latest extraction's write-relevant ``(kind, title)`` pairs.
        ...
        never become a write on this patient's chart.
        """
        ...
        writable: list[tuple[WriteKind, str]] = []
        for row in rows:
