...
def build_observability(settings: Settings) -> Observability:
    ...
    return LangfuseObservability(
        ...
        # Keys the patient-id pseudonyms in every span/event this backend ships.
        # Empty ⇒ patient_id is omitted from trace metadata rather than sent raw
        # (see copilot.observability.pseudonymize).
        pseudonym_key=settings.observability_pseudonym_key,
    )
