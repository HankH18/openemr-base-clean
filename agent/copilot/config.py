...
class Settings(BaseSettings):
    ...
    doc_extraction_confidence_threshold: float = Field(
        ...
        description=(
            ...
            "confidence (its own 'I could not read this') is still withheld. Setting "
            "0.0 does NOT drop the guard entirely: reconcile_value picks the winner by "
            "similarity*min_conf > best_score (seeded at 0.0), so a literal "
            "zero-confidence token is over-withheld even at 0.0 — what 0.0 changes vs "
            "the 0.01 floor is only the (0, 0.01) band, where coverage + similarity "
            "then decide. Values below the floor "
